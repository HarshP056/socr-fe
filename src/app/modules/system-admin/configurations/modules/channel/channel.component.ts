...
export class ChannelComponent implements OnInit {
  ...
  @ViewChild('addFileModal', { static: false })
  addFileModal: BasicModalComponent;
  @ViewChild('addFileView', { static: false })
  addFileView: TemplateRef<any>;
  editChannel: string;
  getChannel: any;
  ...
  goToRestBasedChannel() {
    this.router.navigateByUrl(
      '/system-admin/configurations/channel/rest-based-channel'
    );
  }

  goToLocalFileChannel() {
    this.router.navigateByUrl(
      '/system-admin/configurations/channel/local-file-channel'
    );
  }
  goToNetworkFileChannel() {
    this.router.navigateByUrl(
      '/system-admin/configurations/channel/network-file-channel'
    );
  }

  goToFTPFileChannel() {
    this.router.navigateByUrl(
      '/system-admin/configurations/channel/ftp-file-channel'
    );
  }

  EditChannelType(element: any) {
    this.router.navigateByUrl(
      '/system-admin/configurations/channel/edit-channel/' + element.id
    );
  }
  ...
  private getArgsObj() {
    ...
    if (this.queryParams.channel) obj['type'] = this.queryParams.channel;
    return obj;
  }
  ...
  private getAllChannels() {
    this.channelService.getCannelByPage(this.getArgsObj()).subscribe({
      next: (res: any) => {
        ...
        this.dataSource = res['content'].map((elem: any) => {
          const index = this.selectedChannel.findIndex(
            (val: any) => val.id === elem.id
          );
          if (index > -1) {
        ...
        this.autoFixDefaults(this.channels);
  ...
  duplicate(element: any){
    this.router.navigateByUrl(
      '/system-admin/configurations/channel/copy-channel/' + element.id
    );
  }
  ...
  export(){
    ...
    let qry: any = {
      usernameId : this.queryParams.username,
      type : this.queryParams.type
    }
    let array : any = [];
    if(this.selectedChannel?.length){
      array = this.selectedChannel.map((elem:any) => elem.id)
    }
    this.channelService.exportChannel(qry, array).subscribe({
      next: (res: any) => {
        this.isDownload = false;
        console.log(res, 'res');
        if (res instanceof Blob) {
          saveAs(res, 'Channel.zip');
  ...
  upload(){
    this.addFileModal.templateRef = this.addFileView;
  ...
  onElementSelect(val: any) {
    const index = this.selectedChannel.findIndex(
      (elem: any) => elem.id === val.id
    );
    if (index > -1) {
      this.selectedChannel.splice(index, 1);
    } else {
      this.selectedChannel.push(val);
    }
    this.checkAllChannelSelected();
  }

  private checkAllChannelSelected() {
    let isAllSelected: boolean = true;
    let exportButton: boolean = false;
  ...
  private autoFixDefaults(channels: any[]) {
    channels.forEach((channel: any) => {
      if (!channel.docIdLogicType || !channel.channelType || !channel.timeZone) {
        if (!channel.docIdLogicType) channel.docIdLogicType = 'Common Logic';
        if (!channel.channelType)    channel.channelType    = 'Basic';
        if (!channel.timeZone)       channel.timeZone       = 'Asia/Calcutta';

        this.channelService.putChannel({ ...channel }, channel.id).subscribe();
      }
    });
  }
