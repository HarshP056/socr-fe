...
@Component({
  selector: 'app-create-ocr-project',
  templateUrl: './create-ocr-project.component.html',
  styleUrls: ['./create-ocr-project.component.scss']
})
export class CreateOcrProjectComponent implements OnInit {
  ...
  submit() {
    ...
    //console.log( this.ocrProfileObject.items)
    //console.log(this.ocrProfileObject)

    // this.ocrProfileObject.lineMappings.forEach((line: any) => {
    //   for (let index = 0; index < this.ocrProfileObject.items.length; index++) {
    //     const item = this.ocrProfileObject.items[index];
    //     if(item.category == 'L' && line.dataField == item.dataField) {
  ...
  showLLMRuleDetails(data: any) {
    this.showLLMRuleList = false;
     if(data == null || !data){
      this.llmRuleObj.ruleName = '',
      this.llmRuleObj.vendorId = '',
      this.llmRuleObj.vendorName = '',
      this.llmRuleObj.lineEnabled = false
    }
    else{
  ...
  saveLLMRule() {
    let data:any = {
      ...
      projectId: this.ocrProfileObject.projectId,
      vendorId: this.llmRuleObj.vendorId ? this.llmRuleObj.vendorId : '*',
      vendorName: this.llmRuleObj.vendorName,
      userPrompts: this.llmRuleObj.userPrompt,
      lineEnabled: this.llmRuleObj.lineEnabled
    };
    if(this.llmRuleObj.id){
      data['id'] = this.llmRuleObj.id 
    }
    else{
      data['id'] = "" 
    }
  ...
  deleteLLMRule() {
    ...
    this.ocrProjectService.deleteLLMRule(this.deleteObj.id).subscribe({
      next: (res: any) => {
        this.isDelete = false;
        this.getLLMRulesPage();
